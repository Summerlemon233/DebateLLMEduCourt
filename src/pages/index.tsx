...
export default function HomePage() {
  ...
  const [verifierModel, setVerifierModel] = useState<string>('gemini');
  ...
  const handleModelChange = (models: string[]) => {
    ...
    // 如果当前验证者模型不在选择列表中，自动切换到第一个可用的验证者模型
    if (!models.includes(verifierModel)) {
      const availableVerifier = DEFAULT_MODELS.find(m => 
        models.includes(m.id) && m.isVerifier
      );
      if (availableVerifier) {
        setVerifierModel(availableVerifier.id);
      }
    }
  };

  // 处理验证者模型变化
  const handleVerifierChange = (modelId: string) => {
    setVerifierModel(modelId);
  };
          ...
          <Content className="app-content">
            ...
            <div className="model-selector-section">
              <ModelSelector
                ...
                onVerifierChange={handleVerifierChange}
